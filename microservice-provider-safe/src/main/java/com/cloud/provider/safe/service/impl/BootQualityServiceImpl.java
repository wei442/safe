...
public class BootQualityServiceImpl implements IBootQualityService {
	...
	public Integer deleteQualityById(Integer id) {
  		logger.info("(BootQualityService-deleteQualityById)-根据id删除资质-传入参数, id:{}", id);
  		int i = 0;
  		try {
  			i = qualityMapper.deleteByPrimaryKey(id);
  		} catch (Exception e) {
  			logger.error("(BootQualityService-deleteQualityById)-根据id删除资质-事务性异常, Exception = {}, message = {}", e, e.getMessage());
  			throw new SafeException(SafeResultEnum.SYSTEM_ERROR);
  		}
  		if(i<=0) {
  			throw new SafeException(SafeResultEnum.DATABASE_ERROR);
