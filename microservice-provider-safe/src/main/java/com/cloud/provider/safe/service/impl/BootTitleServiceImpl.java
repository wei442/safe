...
public class BootTitleServiceImpl implements IBootTitleService {
	...
	public Integer deleteTitleById(Integer id) {
  		logger.info("(BootTitleService-deleteTitleById)-根据id删除职务-传入参数, id:{}", id);
  		int i = 0;
  		try {
  			i = titleMapper.deleteByPrimaryKey(id);
  		} catch (Exception e) {
  			logger.error("(BootTitleService-deleteTitleById)-根据id删除职务-事务性异常, Exception = {}, message = {}", e, e.getMessage());
  			throw new SafeException(SafeResultEnum.SYSTEM_ERROR);
  		}
  		if(i<=0) {
  			throw new SafeException(SafeResultEnum.DATABASE_ERROR);
