...
public class BootDictItemServiceImpl implements IBootDictItemService {
	...
	public Integer deleteDictItemById(Integer id) {
  		logger.info("(BootDictItemService-deleteDictItemById)-根据id删除字典子项-传入参数, id:{}", id);
  		int i = 0;
  		try {
  			i = dictItemMapper.deleteByPrimaryKey(id);
  		} catch (Exception e) {
  			logger.error("(BootDictItemService-deleteDictItemById)-根据id删除字典子项-事务性异常, Exception = {}, message = {}", e, e.getMessage());
  			throw new SafeException(SafeResultEnum.SYSTEM_ERROR);
  		}
  		if(i<=0) {
  			throw new SafeException(SafeResultEnum.DATABASE_ERROR);
