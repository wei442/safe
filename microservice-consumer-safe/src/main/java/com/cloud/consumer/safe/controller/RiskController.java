...
import com.cloud.consumer.safe.rest.request.dict.RiskIdRequest;
import com.cloud.consumer.safe.rest.request.dict.RiskRequest;
import com.cloud.consumer.safe.rest.request.page.dict.RiskPageRequest;
import com.cloud.consumer.safe.service.IRiskService;
import com.cloud.consumer.safe.validator.group.UpdateGroup;
import com.cloud.consumer.safe.vo.base.BasePageResultVo;
import com.cloud.consumer.safe.vo.base.BaseResultVo;
import com.cloud.consumer.safe.vo.dict.RiskVo;

import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;

/**
 * 字典子项管理 RiskController
 * @author wei.yong
...
public class RiskController extends BaseController {
	...
	public BaseRestMapResponse getListByPage(
		...
		logger.info("===step2:【分页查询】(RiskController-getListByPage)-分页查询字典子项列表, jsonRisk:{}", jsonRisk);
	...
	public BaseRestMapResponse getList(
		...
		logger.info("===step2:【不分页查询】(RiskController-getList)-不分页查询字典子项列表, jsonRisk:{}", jsonRisk);
		String dataListStr = JSONObject.toJSONString(jsonRisk.getJSONArray(PageConstants.DATA_LIST));
		List<RiskVo> riskVoList  = JSONObject.parseObject(dataListStr, new TypeReference<List<RiskVo>>(){});
	...
	public BaseRestMapResponse getDetail(
		...
		logger.info("===step1:【获取字典子项】(RiskController-getDetail)-请求参数, req:{}, json:{}", req, JSONObject.toJSONString(req));

		Integer riskId = req.getRiskId();
		JSONObject jsonRisk = riskService.getById(riskId);
		logger.info("===step2:【获取字典子项】(RiskController-getDetail)-根据riskId获取字典子项, jsonRisk:{}", jsonRisk);
		RiskVo riskVo = JSONObject.toJavaObject(jsonRisk, RiskVo.class);

		//返回信息
		BaseRestMapResponse riskResponse = new BaseRestMapResponse();
		riskResponse.put(CommConstants.RESULT, riskVo);
	    logger.info("===step3:【获取字典子项】(RiskController-getDetail)-返回信息, riskResponse:{}", riskResponse);
	    return riskResponse;
	}
	...
	public BaseRestMapResponse add(
		...
		BindingResult bindingResult) {
		logger.info("===step1:【新增字典子项】(RiskController-add)-请求参数, req:{}, json:{}", req, JSONObject.toJSONString(req));

		JSONObject jsonRisk = riskService.add(req);
		logger.info("===step2:【新增字典子项】(RiskController-add)-分页查询字典子项列表, jsonRisk:{}", jsonRisk);

		//返回信息
		BaseRestMapResponse riskResponse = new BaseRestMapResponse();
	    logger.info("===step3:【新增字典子项】(RiskController-add)-返回信息, riskResponse:{}", riskResponse);
	    return riskResponse;
	}
	...
	public BaseRestMapResponse delete(
		@Validated @RequestBody RiskIdRequest req,
		BindingResult bindingResult) {
		logger.info("===step1:【删除字典子项】(RiskController-delete)-请求参数, req:{}, json:{}", req, JSONObject.toJSONString(req));

		Integer riskId = req.getRiskId();
		JSONObject jsonRisk = riskService.deleteById(riskId);
		logger.info("===step2:【删除字典子项】(RiskController-delete)-根据riskId删除字典子项, jsonRisk:{}", jsonRisk);

		//返回信息
		BaseRestMapResponse riskResponse = new BaseRestMapResponse();
		logger.info("===step3:【删除字典子项】(RiskController-delete)-返回信息, riskResponse:{}", riskResponse);
		return riskResponse;
	}
	...
	public BaseRestMapResponse update(
		...
		BindingResult bindingResult) {
		logger.info("===step1:【修改字典子项】(RiskController-update)-请求参数, req:{}, json:{}", req, JSONObject.toJSONString(req));

		JSONObject jsonRisk = riskService.update(req);
		logger.info("===step2:【修改字典子项】(RiskController-update)-修改字典子项, jsonRisk:{}", jsonRisk);

		//返回信息
		BaseRestMapResponse riskResponse = new BaseRestMapResponse();
		logger.info("===step3:【修改字典子项】(RiskController-update)-返回信息, riskResponse:{}", riskResponse);
		return riskResponse;
	}
